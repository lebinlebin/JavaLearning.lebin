...
枚举可能分割位置：即如果 s[0:i] 本身不是一个回文串，枚举分割的边界 j。
1. 如果 s[j + 1, i] 不是回文串，尝试下一个分割边界。
2. 如果 s[j + 1, i] 是回文串，则 dp[i] 就是在 dp[j] 的基础上多一个分割。
于是枚举 j 所有可能的位置，取所有 dp[j] 中最小的再加 1 ，就是 dp[i]。
===> 状态转移方程：dp[i] = min([dp[j] + 1 for j in range(i) if s[j + 1, i] 是回文])
*/
    /*
    时间复杂度：O(n^2)，
    空间复杂度：O(n)
    ...
    public int minCut2(String s) {
        ...
        for (int right = 0; right < len; right++) {
            ...
            for (int left = 0; left <= right; left++) {
                //right - left <= 2：子串长度小于或等于3。由于我们已经知道两个端点字符相同，因此长度为3或更短的子串自然是回文（在只考虑这两个端点字符的情况下）。
                //checkPalindrome[left + 1][right - 1]：除去两个端点字符后的子串s[left+1...right-1]也必须是回文。这是通过查找之前计算并存储在checkPalindrome数组中的信息来确定的。
