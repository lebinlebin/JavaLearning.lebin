...
思路总结:
1.通过异或方法，找到两个只出现一次的两个数的异或值 x^y
2. 找到x^y中最低位的1，因为x!=y,因此，一定至少有一位位1，对应最低位为1的变量记为m
3. 通过m与nums中所有数字进行位相与，一定会把x和y分成两组。一遍相与结果为0；一边相与结果为1；
此时问题转化为一个数组，有一个数只出现一次，其他数出现了两次的问题。直接全员异或得到的结果就是我们要求的。
4. 两组数中，所有元素进行异或，就得到x和y
*/
...
//    空间复杂度 O(1)
    public int[] singleNumbers(int[] nums) {
        ...
        int z = 0;
        for(int i : nums) {z ^= i;}
        //因为x和y是不相等的，因此二进制中一定有一位是不相等的。不同的。
        //异或的性质：z中至少有一位是1，否则x与y就是相等的。
        //通过辅助变量m来保存z中哪一位为1.（可能有多个位都为1，找到最低位的1即可）。举个例子：z = 10 ^ 2 = 1010 ^ 0010 = 1000,第四位为1.
        //将m初始化为1，如果（z & m）的结果等于0说明z的最低位是0
        //每次将m左移一位然后跟z做与操作，直到结果不为0.
